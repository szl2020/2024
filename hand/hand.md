...
1. 审题
   ...
   `js
    ...
    `
   3）根据注释写代码
   4）总结 => a.数组 => b.对象转化 => c.hash 匹配 => d.对于树级操作

2. 面试题举例
   a. 实现函数的防抖和节流？ \*
   防抖 => 什么是防抖？ => 场景：一些点击请求上
   1）函数可以使事件被触发 n 秒之后再进行处理 2) n 秒内事件再次被触发则重新计时
...
1. 既然已经用到了 apply，了解如何实现的吗？如何手写实现一下？ \*
...
function sumBig(a, b) {
  ...
  // 条件为至少有存留数
  let needCalc = a.length || b.length || _tmp;
